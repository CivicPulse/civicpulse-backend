...
from civicpulse.models import User, Person, ContactAttempt, VoterRecord, PasswordHistory
...
class MigrationTestDataGenerator:
    ...
    def create_persons(self, count=10000):
        ...
        for i in range(count):
            person_data = {
                ...
                'last_name': self.fake.last_name(),
                'suffix': random.choice(['Jr.', 'Sr.', 'III', '']) if random.random() > 0.9 else '',
                'date_of_birth': self.fake.date_of_birth(minimum_age=18, maximum_age=90) if random.random() > 0.3 else None,
                'gender': random.choice(['M', 'F', 'O', 'U']),
                'email': self.fake.email() if random.random() > 0.2 else '',
                'phone_primary': self.fake.phone_number()[:20] if random.random() > 0.1 else '',
                'phone_secondary': self.fake.phone_number()[:20] if random.random() > 0.8 else '',
                'street_address': self.fake.street_address() if random.random() > 0.1 else '',
                'apartment_number': self.fake.secondary_address() if random.random() > 0.7 else '',
                ...
                'notes': self.fake.text(max_nb_chars=200) if random.random() > 0.6 else '',
    ...
    def create_contact_attempts(self, count=50000):
        ...
        for i in range(count):
            contact_data = {
                ...
                'contact_type': random.choice(['phone', 'text', 'email', 'door', 'mail', 'social', 'event', 'other']),
                'contact_date': timezone.make_aware(self.fake.date_time_between(start_date='-2y', end_date='now')),
                'result': random.choice(['contacted', 'no_answer', 'left_message', 'wrong_number', 'refused', 'callback', 'not_home', 'moved', 'deceased', 'other']),
                'sentiment': random.choice(['strong_support', 'support', 'neutral', 'oppose', 'strong_oppose', 'undecided', '']),
                'issues_discussed': [self.fake.word() for _ in range(random.randint(0, 3))],
                'commitments': [self.fake.word() for _ in range(random.randint(0, 2))],
                'follow_up_required': random.choice([True, False]),
                'follow_up_date': self.fake.future_date() if random.random() > 0.7 else None,
                'notes': self.fake.text(max_nb_chars=300) if random.random() > 0.5 else '',
                'duration_minutes': random.randint(1, 60) if random.random() > 0.3 else None,
                'campaign': f'Campaign_{random.randint(1, 10)}' if random.random() > 0.4 else '',
                'event': f'Event_{random.randint(1, 20)}' if random.random() > 0.7 else '',
            }
    ...
    def create_voter_records(self, count=8000):
        ...
        for i, person in enumerate(persons_without_voters):
            voter_data = {
                ...
                'registration_date': self.fake.date_between(start_date='-10y', end_date='now') if random.random() > 0.1 else None,
                'registration_status': random.choice(['active', 'inactive', 'pending', 'cancelled', 'suspended']),
                'party_affiliation': random.choice(['DEM', 'REP', 'IND', 'GRN', 'LIB', 'OTH', 'NON']),
                'precinct': f'P{random.randint(1, 50)}' if random.random() > 0.2 else '',
                'ward': f'W{random.randint(1, 20)}' if random.random() > 0.3 else '',
                'congressional_district': f'CD{random.randint(1, 15)}' if random.random() > 0.1 else '',
                'state_house_district': f'HD{random.randint(1, 100)}' if random.random() > 0.1 else '',
                'state_senate_district': f'SD{random.randint(1, 50)}' if random.random() > 0.1 else '',
                'polling_location': self.fake.address() if random.random() > 0.3 else '',
                'voting_history': [{'year': y, 'election': f'General_{y}', 'voted': random.choice([True, False])} 
                                 for y in range(2020, 2025)],
                'absentee_voter': random.choice([True, False]),
                'mail_ballot_requested': random.choice([True, False]),
                'mail_ballot_sent_date': self.fake.date_between(start_date='-1y', end_date='now') if random.random() > 0.7 else None,
                'mail_ballot_returned_date': self.fake.date_between(start_date='-1y', end_date='now') if random.random() > 0.8 else None,
                'last_voted_date': self.fake.date_between(start_date='-4y', end_date='now') if random.random() > 0.2 else None,
                'voter_score': random.randint(0, 100),
                'source': random.choice(['State_Rolls', 'Party_Lists', 'Manual_Entry', 'Import_2024']),
            }
    ...
    def create_password_history(self, count=500):
        ...
        with transaction.atomic():
            PasswordHistory.objects.bulk_create(password_history_to_create, batch_size=1000)
    ...
    def create_audit_logs(self, count=25000):
        ...
        for i in range(count):
            ...
            audit_data = {
                ...
                'action': random.choice(['CREATE', 'UPDATE', 'DELETE', 'VIEW', 'LOGIN', 'LOGOUT', 'EXPORT', 'IMPORT']),
                'category': random.choice(['VOTER_DATA', 'AUTH', 'SYSTEM', 'CONTACT', 'ADMIN', 'SECURITY']),
                ...
                'changes': {'field1': {'old': 'old_value', 'new': 'new_value'}} if random.random() > 0.3 else {},
                'old_values': {'field1': 'old_value', 'field2': 'old_value2'} if random.random() > 0.5 else {},
                'new_values': {'field1': 'new_value', 'field2': 'new_value2'} if random.random() > 0.5 else {},
                ...
                'metadata': {'key1': 'value1', 'count': random.randint(1, 100)} if random.random() > 0.4 else {},
                'search_vector': self.fake.text(max_nb_chars=100) if random.random() > 0.5 else '',
            }
    ...
    def generate_all_data(self):
        ...
        self.create_users(100)          # 100 users
        self.create_persons(10000)      # 10k persons (main entities)
        self.create_contact_attempts(50000)  # 50k contacts (high volume)
        self.create_voter_records(8000)     # 8k voter records
        self.create_password_history(500)   # 500 password history
        self.create_audit_logs(25000)       # 25k audit logs (high volume)
    ...
    def cleanup_test_data(self):
        ...
        ContactAttempt.objects.filter(person__created_by__username__startswith='testuser_').delete()
        VoterRecord.objects.filter(person__created_by__username__startswith='testuser_').delete()
