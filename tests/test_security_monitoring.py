...
class TestFailedLoginDetection(TestCase):
    ...
    def test_threshold_exceeded_triggers_alert(self):
        ...
        critical_logs = AuditLog.objects.filter(
            severity=AuditLog.SEVERITY_CRITICAL, category=AuditLog.CATEGORY_SECURITY
        )
    ...
    def test_time_window_filtering(self):
        ...
        result = check_failed_login_attempts(
            ip_address=self.ip_address, username=self.username, window_hours=1
        )
    ...
    def test_exception_handling(self):
        ...
        with patch(
            "civicpulse.audit.AuditLog.objects.filter",
...
class TestUnusualExportActivityDetection(TestCase):
    ...
    def test_custom_threshold_and_window(self):
        ...
        result = detect_unusual_export_activity(
            user=self.user, threshold=2, window_hours=24
        )
    ...
    def test_time_window_filtering(self):
        ...
        result = detect_unusual_export_activity(user=self.user, window_hours=24)

        # Should only count recent exports (3), not old one
        self.assertEqual(result["export_count"], 3)
        self.assertEqual(
            result["total_records_exported"], 300
        )  # Not including old 1000
...
class TestSecurityDashboard(TestCase):
    ...
    def test_dashboard_with_security_events(self):
        ...
        self.assertEqual(
            result["unique_failed_login_ips"], 5
        )  # 3 from critical + 2 unique from regular
