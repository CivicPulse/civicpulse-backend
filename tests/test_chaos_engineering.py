...
class NetworkPartitionTests(ChaosEngineeringTestCase):
    ...
    def test_network_latency_simulation(self):
        ...
        with patch('requests.get') as mock_get:
            ...
            self.assertGreater(response_time, baseline_avg * 2, 
    ...
    def test_intermittent_connectivity(self):
        ...
        logger.info(f"Success rate during intermittent connectivity: {success_rate:.2%}")
        
        # System should recover when connectivity returns
        self.assertGreater(success_rate, 0.5, "System should have some successful requests")
...
class ContainerRestartResilienceTests(ChaosEngineeringTestCase):
    ...
    @pytest.mark.skipif(not os.getenv('DOCKER_TESTING'), 
                       reason="Docker tests require DOCKER_TESTING=1")
    def test_graceful_shutdown_signal_handling(self):
        ...
        try:
            ...
            self.assertTrue(signal_received, "Application should handle shutdown signals")
    ...
    def test_concurrent_startup_requests(self):
        ...
        self.assertGreater(success_rate, 0.3, "Should handle concurrent startup requests")
    ...
    def test_graceful_degradation_scenarios(self):
        ...
        for scenario_name, setup_func in scenarios:
            with self.subTest(scenario=scenario_name):
                ...
                logger.info(f"Graceful degradation ({scenario_name}): {healthy}, {result}")
...
class SystemRecoveryTests(ChaosEngineeringTestCase):
    ...
    def test_automatic_recovery_after_failures(self):
        ...
        for failure_func in failure_scenarios:
            with self.subTest(failure=failure_func.__name__):
                ...
                healthy, result = self.make_health_check()
                logger.info(f"Recovery test ({failure_func.__name__}): {healthy}, {result}")
    ...
    def test_recovery_time_measurement(self):
        ...
        failure_scenarios = {
            'database_timeout': lambda: patch('django.db.connection.cursor', 
                                            side_effect=Exception("DB timeout")),
            'cache_unavailable': lambda: patch.object(cache, 'get', 
        ...
        for scenario, recovery_time in recovery_times.items():
            self.assertLess(recovery_time, 30, 
                          f"Recovery for {scenario} should be under 30 seconds")
...
class PerformanceUnderStressTests(ChaosEngineeringTestCase):
    ...
    def test_concurrent_request_handling(self):
        ...
        for concurrency in concurrency_levels:
            ...
            logger.info(f"Concurrency {concurrency}: {success_rate:.2%} success, "
    ...
    def test_memory_leak_detection(self):
        ...
        for i in range(100):
            ...
            if i % 10 == 0:
                ...
                logger.info(f"Memory usage after {i} requests: "
